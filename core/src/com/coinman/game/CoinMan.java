...
public class CoinMan extends ApplicationAdapter {
	...
	public void render () {
		...
		if(gameState == 2)
		{
			batch.draw(dizzy,(Gdx.graphics.getWidth()/2 - man[manState].getWidth()/2)-200,manY);
		}
		else
		{
			batch.draw(man[manState],(Gdx.graphics.getWidth()/2 - man[manState].getWidth()/2)-200,manY);

		}

		manRectangle = new Rectangle((Gdx.graphics.getWidth()/2 - man[manState].getWidth()/2) -200, manY,100, 100);

		for(int i = 0; i<coinRectangle.size();i++)
		{
			if(Intersector.overlaps(coinRectangle.get(i),manRectangle))
			{
				//Gdx.app.log("Coin","A coin has been captured");
				Gdx.app.log("Overlapper running","match with number of coins");
				score++;
